...
import {
  ...
  createTicket,
  createTicketTypeDoesNotIncludeHotel,
  createTicketTypeIsRemote,
  createTicketTypePerfect,
  createUser,
} from "../factories";
...
describe("GET /hotels/id", () => {
  ...
  describe("when token is valid", () => {
    ...
    it("should respond with status 404 if there are no rooms", async () => {
      ...
      const response = await server.get(`/hotels/${hotel1.id}`).set("Authorization", `Bearer ${token}`);
    ...
    it("should respond with status 404 if hotelId is not a valid Id", async () => {
      ...
      const response = await server.get("/hotels/10924812").set("Authorization", `Bearer ${token}`);
      expect(response.status).toEqual(httpStatus.NOT_FOUND);
    });
